...
export function DeductionEdit({onEditToggle}: DeductionEditProps) {
	const {selectedDataByType} = useSelectedRowsStore();
	const [loading, setLoading] = useState(false);
	const {selectedEmployee} = useEmployeeStore();
	const defaultValues = {
		deductions: selectedDataByType['deductions']?.length
			? selectedDataByType['deductions']
			: [
					{
						name: '',
						start: '',
	...
	const processForm = async (data: DeductionAddMore) => {
		...
		try {
			const deductionRequest = data.deductions.map((deduction) => {
				...
				const existingBenefit = selectedDataByType['deductions'].find(
					(item) => item.deduction_id === deduction.deduction_id,
				);

				if (existingBenefit?.deduction_id) {
					// Update existing benefit
					return request(
						'PUT',
						`/api/v1/ems/employees/${selectedEmployee?.employee_id}/deductions/${existingBenefit.deduction_id}`,
						{
							name: deduction.name,
