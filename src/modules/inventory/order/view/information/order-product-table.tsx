...
import useOrderProductStore from '../../_components/use-orderProduct';
...
export function OrderProductTable<TData extends OrderProduct, TValue>({
	columns,
	data,
	pageCount,
	...
	useEffect(() => {
		if (data.length > 0) {
			const rdata: OrderProduct = data[0];
			useOrderProductStore.getState().setOrderProduct(rdata);
		}
	}, [data]);

	const handleRowClick = (row: Row<TData>) => {
		const rowData: OrderProduct = row.original;

		useOrderProductStore.getState().setOrderProduct(rowData);
	};
