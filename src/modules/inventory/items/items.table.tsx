...
export function ItemsWithDetailsTable<TData extends ItemWithDetails, TValue>({
	columns,
	data,
	pageCount,
	searchKey,
	...
	useEffect(() => {
		if (data.length > 0) {
			const rdata: ItemWithDetails = data[0];
			useItemsWithDetailsStore.getState().setItemsWithDetals(rdata);
		}
	}, [data]);
	...
	const handleRowClick = (row: Row<TData>) => {
		...
		const rowData: ItemWithDetails = row.original;

		// Do something with the row data
		useItemsWithDetailsStore.getState().setItemsWithDetals(rowData);
	};
